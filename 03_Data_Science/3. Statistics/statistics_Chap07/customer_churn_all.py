...
# Specifiy different statistics for different variables
print(churn.groupby(['churn']).agg({'day_charge': ['mean', 'std'],
                                    'eve_charge': ['mean', 'std'],
                                    'night_charge': ['mean', 'std'],
                                    'intl_charge': ['mean', 'std'],
                                    'account_length': ['count', 'mean', 'std'],
                                    'custserv_calls': ['count', 'mean', 'std']}))
...
# for each of the quartiles, and add them to the churn DataFram
...
print("Probablility of churn when account length changes by 1 : %.2f" % (inverse_logit(cust_serv_mean) - inverse_logit(cust_serv_mean_minus_one)))
...
logit_model = sm.Logit(output_variable, input_variables).fit_regularized()
# logit_model = smf.glm(output_variable, input_variables, family=sm.families.Binomial()).fit()
# print(logit_model.summary())
